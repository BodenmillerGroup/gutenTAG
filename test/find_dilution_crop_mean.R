...
# 2. Read in data
Path_to_imzml_file <- "/mnt/msi_volume/experiments/080523_dilution/14.imzML"
rawFile <- readMSIData(Path_to_imzml_file)

# 3. Create custom peakAnnotation df ####
peakAnnotation <- data.frame("uncharged_peptide_plus1" = 919.584, 
                             "charged_peptide_plus1" = 961.628, 
                             "uncharged_peptide_plus2" = 460.298, 
                             "charged_peptide_plus2" = 481.335)
peakAnnotation <- t(peakAnnotation)

colnames(peakAnnotation) <- c("Name", "Mass")
...
Plot_channel(my_df, 4)


# 7. Crop intensity dataframe according to cropped pixel coordinates ####
...
Plot_channel_cropped = function(image, channel_number=1, quantile_lim = 0.99) {
