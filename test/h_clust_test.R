...
cut_init <- as.numeric(cutree(h_cluster, k = 15, h = NULL))

#classes <- list()
#for (i in length(table(cut_init))){
# 5
#  class_temp <- as.numeric(which(cut_init == i))
#  classes[[i]] <- class_temp
#  
#}

dunn(distance_matrix, cut_init)


class1 <- as.numeric(which(cut_init == 1))
