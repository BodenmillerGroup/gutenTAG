...
# Find peaks function written by 
cat("Loading of the functions... ")
...
#Path_to_imzml_file = "mnt/msi_volume/experiments/QC-report-ImzML/menzha_20221215_tonsil_resublimate_20um.imzML"
rawFile = readMSIData(Path_to_imzml_file)
cat("done ! \n")


cat("Subsetting if needed... ")
rawFile <- subsetPixels(rawFile, x<=250, y<= 200)
cat("done ! \n")
...
#Path_to_peak_annotation = "mnt/msi_volume/experiments/QC-report-ImzML/SciLS_library/25-plex-real.csv"
...
# Functions for plotting ion images nicely

cat("Creating plotting channel functions... ")
...
Plot_channel = function(channel_number=1,quantile_lim = 0.99) {
  ...
  plot((Matrix_image),main=Correspondence_matrix$Annotation_peaks[channel_number])
}
...
Plot_two_channel = function(channel_1=1,channel_2=2,quantile_lim = 0.99) {
  ...
  x = Final_intensity_matrix[,channel_1]
  x_max = quantile(x,probs = quantile_lim)
  x[x>x_max]=x_max
  ...
  x = Final_intensity_matrix[,channel_2]
  x_max = quantile(x,probs = quantile_lim)
  x[x>x_max]=x_max
...
Plot_three_channel = function(channel_1=1,channel_2=2,channel_3=3,quantile_lim = 0.99){
  ...
  x = Final_intensity_matrix[,channel_3]
  x_max = quantile(x,probs = quantile_lim)
  x[x>x_max]=x_max
...
# b). Standard summary statistics (mean, variance, corrected standard deviation)
...

{
  ...
  plot(Mean_signal, Sd_signal, log="xy", 
       pch=21, bg=string.to.colors(is.na(Correspondence_matrix$Annotation_peaks), 
                                   colors = c("grey","red3")),
...
# show all ion images 
for (n in 1:nrow(Correspondence_matrix)){
...
write.table(raw_intensity, file = paste(sample_path,"/", Sample_name, "raw_intensity_data.txt", sep = ""), sep="\t", quote = F, row.names = F)
print("Location data")
...
print("Final Intnensity Matrix")
write.table(Final_intensity_matrix, file = paste(sample_path,"/", Sample_name, "_export_table.txt", sep = ""), sep="\t", quote = F, row.names = F)
