...
- Replace `cbind` growing loop in `.generateFinalIntensityDF` — pre-allocated `matrix(0, nrow = n_pixels, ncol = n_metapeaks)` and replaced `cbind` with index assignment `final_intensity[, k]`; added comment explaining `raw_intensity` orientation
- Replace column loop in `.filterTIC` with vectorised assignment — replaced per-column `for` loop with single `as.matrix()` + `final_filtered[-indices, ] <- NA`; note the value-membership bug was already fixed in a prior session
- Vectorise `cor` computation in `.finaliseCorrespondence` — replaced `apply(..., FUN = function(x) cor(...))` with `drop(cor(log(final_intensity + 1), log(1 + total_signal)))`
- Add input validation to `generateSeedMz` for `density` parameter — added guard `!is.numeric(density) || length(density) != 1 || density <= 0` with `stop()`; added `@param density` roxygen entry
- Expand tests for `computeGearysC` — added second `test_that` block with synthetic fixture covering: return vector length, zero-variance channel returns `NA_real_` not `NaN`, all-NA channel returns `NA_real_` not `NaN`, `update_correspondence = TRUE` vs `FALSE` return identical scores
- Investigate `'x' should be a list object` error in `assignMetapeaks` — root cause was stale function name in error message (`metapeakGeneration` → `generateMetapeaks`); added field check for `metapeaks` and `propagation_selection`; updated all `@param`/`@return` roxygen docs in `assignMetapeaks.R`; fixed section comment in `validityChecks.R`
